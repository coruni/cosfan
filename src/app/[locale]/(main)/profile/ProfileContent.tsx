...
export default function ProfileContent() {
  ...
  const { data: browseHistory } = useQuery({
    ...
    queryFn: async () => {
      const response = await articleControllerGetUserBrowseHistory({
  ...
  const { data: likedArticles } = useQuery({
    ...
    queryFn: async () => {
      const response = await articleControllerGetLikedArticles({
        query: { page: currentPage, limit: 20 },
      });
      return response.data?.data || { data: [], meta: { total: 0 } };
    },
  ...
  const { data: favoritedArticles } = useQuery({
    ...
    queryFn: async () => {
      const response = await articleControllerGetFavoritedArticles({
        query: { page: currentPage, limit: 20 },
      });
      return response.data?.data || { data: [], meta: { total: 0 } };
    },
            ...
            {getPageNumbers(currentPage, totalPages).map((page, index) => (
              <PaginationItem key={index}>
                {page === "ellipsis" ? (
                  ...
                  <PaginationLink
                    href={createTabUrl(currentTab, page)}
        ...
        <TabsContent value="likes">
          <Card>
            ...
            <CardContent>
              <ArticleGrid
                articles={
                  (likedArticles?.data as ArticleControllerFindAllResponse["data"]["data"]) ||
                  []
                }
              />
        ...
        <TabsContent value="favorites">
          <Card>
            ...
            <CardContent>
              <ArticleGrid
                articles={
                  (favoritedArticles?.data as ArticleControllerFindAllResponse["data"]["data"]) ||
                  []
                }
              />
