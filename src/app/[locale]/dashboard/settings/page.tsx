...
const CONFIG_GROUPS = [
  ...
  },
  {
    ...
    label: "APP设置",
    icon: Smartphone,
    description: "移动应用相关配置",
  },
  {
    value: "user",
    label: "用户设置",
    icon: Users,
    description: "用户注册与验证配置",
  },
  {
    value: "membership",
    label: "会员设置",
    icon: Crown,
    description: "会员价格与权益配置",
  },
  {
    value: "payment",
    label: "支付设置",
    icon: CreditCard,
    description: "支付网关与渠道配置",
  },
  {
    value: "content",
    label: "内容设置",
    icon: FileText,
    description: "文章与评论审核配置",
  },
  {
    value: "advertisement",
    label: "广告设置",
  ...
  },
  {
    ...
    label: "邀请设置",
    icon: UserPlus,
    description: "邀请码与奖励配置",
  },
  {
    value: "commission",
    label: "佣金设置",
    icon: Percent,
    description: "佣金分成比例配置",
  },
  {
    value: "favorite",
    label: "收藏夹设置",
    icon: Heart,
    description: "收藏夹创建费用配置",
  },
  {
    value: "system",
    label: "系统设置",
...
export default function SettingsPage() {
  ...
  const createMutation = useMutation({
    mutationFn: async (data: { key: string; value: string; description?: string; group?: string }) => {
  ...
  const isBooleanConfig = (key: string) => {
    return (
      ...
      key.includes("maintenance_") ||
      key.includes("app_force_update") ||
      key.includes("user_email_verification") ||
      key.includes("user_registration_enabled") ||
                          ...
                          <TableRow key={config.id}>
                            <TableCell>
                              <div className="flex items-center gap-2">
                                ...
                                {config.public && (
                                  <span className="text-xs bg-green-100 text-green-700 px-1 py-0.5 rounded hidden sm:inline">
                                    公开
                                  </span>
                                )}
                              </div>
                ...
                <div className="flex items-center gap-3">
                  {editForm.value ? (
                    <img
                      ...
                      alt="预览"
                      className={
                        editForm.key.includes("favicon")
                          ? "w-8 h-8 object-contain border rounded"
                          : "w-16 h-10 rounded object-cover"
                      }
                    />
                  ) : (
